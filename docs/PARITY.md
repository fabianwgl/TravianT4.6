...
| Capital changes | A manual capital switch is atomic, requires an owned non-Wonder village with a Palace, applies both villages' building restrictions, trims incompatible queued work, and remains safe under retries and concurrent construction or destruction | `main_script/include/Model/VillageModel.php`, `main_script/include/Model/MasterBuilder.php`, `main_script/include/Core/Automation.php`, `main_script/include/Core/Jobs/TransactionalTask.php` | `tests/runtime-regression.php` manual-capital fixtures and fresh-process races | verified |
| Town Hall celebrations | Starts atomically charge resources and immediately award the 10× 125/500 CP limits; rewards use theoretical CP including grey-area villages but excluding World Wonders, and active celebrations survive Town Hall destruction | `main_script/include/Model/CelebrationModel.php`, `main_script/include/Controller/Build/TownhallCntrl.php`, `main_script/include/Game/Formulas.php` | `tests/runtime-regression.php` celebration rule, rollback, replay, and fresh-process race fixtures | verified |
